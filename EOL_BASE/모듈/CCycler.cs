...
namespace EOL_BASE.모듈
{
    ...
    public class CCycler// : CSerialModule
    {
        ...
        public bool isAlaramStatusCheck = false;
        ...
        private void tmr_Tick1(object sender, EventArgs e)
        {
            ...
            if (bmsList.ContainsKey("110h"))
            {
                ...
                mw.SetMainVoltage(cycler1voltage.ToString("N2"));

                if (cycler1Header == CMD.DISCHARGE)
                {
                    sb = new StringBuilder();
                    sb.Append("-");
                    sb.Append(cycler1current.ToString("N2"));
                    mw.SetMainCurrent(sb.ToString());
                }
                else
                    mw.SetMainCurrent(cycler1current.ToString("N2"));
            ...
            if (bmsList.ContainsKey("120h"))
            {
                ...
                if (dataWarn == "000000000000" && dataGrid == "000000000000" && data__Dc == "000000000000" && dataFalt == "000000000000")
                {
                    ...
                    //Alarm Flag Add by kyj 200702
                    isAlaramStatusCheck = true;
                }
                else
                {
                    //Alarm Flag Add by kyj 200702
                    isAlaramStatusCheck = false;
                    //////////////////////////////
                    #region DSP_28335
                    if (mw.dsp_Type == DSPType.DSP_28335)
                    {
                        if (errorWarning.Count == 0)
                        {
                            if (dataWarn.Substring(11, 1) == "1") { errorWarning.Add(Lag.lSystem120_28335_W001); }
                            if (dataWarn.Substring(10, 1) == "1") { errorWarning.Add(Lag.lSystem120_28335_W002); }
                            if (dataWarn.Substring(9, 1) == "1") { errorWarning.Add(Lag.lSystem120_28335_W004); }
                            // 190306 JHT - dataWarn.Substring(8, 1) → dataWarn.Substring(4, 1) 수정 (잘못 매칭되어 있던 내용)
                            if (dataWarn.Substring(4, 1) == "1") { errorWarning.Add(Lag.lSystem120_28335_W080); }
                        }

                        if (errorGrid.Count == 0)
                        {
                            if (dataGrid.Substring(11, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G001); }
                            if (dataGrid.Substring(10, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G002); }
                            if (dataGrid.Substring(9, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G004); }
                            if (dataGrid.Substring(7, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G010); }
                            if (dataGrid.Substring(6, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G020); }
                            if (dataGrid.Substring(5, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G040); }
                            if (dataGrid.Substring(3, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G100); }
                            if (dataGrid.Substring(2, 1) == "1") { errorGrid.Add(Lag.lSystem120_28335_G200); }
                        }

                        if (errorDC.Count == 0)
                        {
                            if (data__Dc.Substring(11, 1) == "1") { errorDC.Add(Lag.lSystem120_28335_D001); }
                            if (data__Dc.Substring(10, 1) == "1") { errorDC.Add(Lag.lSystem120_28335_D002); }
                            if (data__Dc.Substring(9, 1) == "1") { errorDC.Add(Lag.lSystem120_28335_D004); }
                            if (data__Dc.Substring(7, 1) == "1") { errorDC.Add(Lag.lSystem120_28335_D010); }
                            if (data__Dc.Substring(6, 1) == "1") { errorDC.Add(Lag.lSystem120_28335_D020); }
                        }

                        if (errorFault.Count == 0)
                        {
                            if (dataFalt.Substring(11, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F001); }
                            if (dataFalt.Substring(10, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F002); }
                            if (dataFalt.Substring(9, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F004); }
                            if (dataFalt.Substring(8, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F008); }
                            if (dataFalt.Substring(7, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F010); }
                            if (dataFalt.Substring(6, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F020); }
                            if (dataFalt.Substring(5, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F040); }
                            if (dataFalt.Substring(4, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F080); }
                            if (dataFalt.Substring(3, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F100); }
                            if (dataFalt.Substring(2, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F200); }
                            if (dataFalt.Substring(1, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F400); }
                            if (dataFalt.Substring(0, 1) == "1") { errorFault.Add(Lag.lSystem120_28335_F800); }
                        }
                    }
                    #endregion
            ...
            if (!is84On)
            {
                ...
                mw.LogState(LogType.Info, logSb.ToString(), null, false, true, true, 1);
            }
        }
        ...
        private string binaryToInt(string d)
        {
        ...
        public static bool AddToSendListBytes(List<SendCMD> slist, SendCMD scmd)
        {
            try
            {
                ...
                switch (scmd.Opmode)
                {
                    ...
                    case OPMode.CHARGE_CP: s = 7 << 4; break;
                    case OPMode.DISCHARGE_CP: s = 8 << 4; break;
                }
                var end = 0;
                //switch (scmd.Pmode)
                //{
        ...
        private void SetPCAN_BMS1(int index, string fname)
        {
            ...
            else
            {
                ...
                SendToDSP1("100", new byte[] { 0x80, 0x01, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 });
                Thread.Sleep(500);
