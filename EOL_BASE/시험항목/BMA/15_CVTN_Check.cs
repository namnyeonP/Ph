...
namespace EOL_BASE
{
    ...
    {        
        public bool CVTN_Delta_Temp(TestItem ti)
        {
            isProcessingUI(ti);
            
            Hybrid_Instru_CAN._635List.Clear();
            Hybrid_Instru_CAN.SendToCAN("735", new byte[] { 0x03, 0x22, 0xD9, 0x80, 0x00, 0x00, 0x00, 0x00 });
            LogState(LogType.Info, "Send 735 { 0x03, 0x22, 0xD9, 0x80, 0x00, 0x00, 0x00, 0x00}");
            ...
            if (Hybrid_Instru_CAN._635List.Count == 1 && Hybrid_Instru_CAN._635List[0].Contains("10 "))
            {
                ...
                var cvtn_tempList = new List<double>();
                for (int i = 4; i < 76; i=i+2)
                {
                    cvtn_tempList.Add((Convert.ToInt32(rList[i] + rList[i+1], 16) / 100) - 50);
                }

                var localTemp = temps.tempStr;
                LogState(LogType.Info, "Ambient Temp:" + localTemp);
                LogState(LogType.Info, "Max Temp:" + cvtn_tempList.Max() + "/Min Temp:" + cvtn_tempList.Min());

                ti.Value_ = cvtn_tempList.Max() - cvtn_tempList.Min();
            }
            else
            {
                foreach (var fe009c in Hybrid_Instru_CAN._635List)
